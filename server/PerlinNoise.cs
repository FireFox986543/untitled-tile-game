namespace server
{
    ...
    */
    ...
    public static class PerlinNoise
    {
        ...
        private static readonly long[] permutation = [
        ...
        public static double Perlin(double x, double y, double z, int repeat = -1)
        {
            ...
            long xi = ((long)Math.Floor(x)) & 255;
            long yi = ((long)Math.Floor(y)) & 255;
            long zi = ((long)Math.Floor(z)) & 255;
            double xf = x - ((long)Math.Floor(x));
            double yf = y - ((long)Math.Floor(y));
            double zf = z - ((long)Math.Floor(z));

            double u = fade(xf);
            double v = fade(yf);
            double w = fade(zf);

            long aaa, aba, aab, abb, baa, bba, bab, bbb;
            aaa = p[p[p[xi] + yi] + zi];
        ...
        static long inc(long num, int repeat)
        {
            num++;
        ...
        static double grad(long hash, double x, double y, double z)
        {
            long h = hash & 15L;
            double u = h < 8 ? x : y;
            double v;
    ...
    public static class Pseudo
    {
        public static double Rand(double s1, double s2, double s3, double max = 1000000)
        {
            ...
            long x = ((long)a) ^ ((long)s1);
            long y = ((long)b) ^ ((long)s2) ^ ((long)s3);
            long z = ((long)c) ^ ((long)a) ^ ((long)(s1 + c)) ^ ((long)s2);
